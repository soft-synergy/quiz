...
function StepContent({
  ...
  const handleContinue = () => {
    if (isInput) {
      ...
      setAnswer(stepNum, textValue.trim())
    } else if (isDateInput && dateValue) {
      setAnswer(stepNum, dateValue)
    }
    navigate('forward')
  }
      ...
      <main className={styles.main}>
        <div className={styles.content}>
          ...
          {(isSingle || isMulti) && stepData.options && (
            <div className={styles.optionsGroup} role={isSingle ? 'radiogroup' : 'group'} aria-labelledby="question-heading">
    ...
    (direction: 'forward' | 'backward') => {
      ...
      if (direction === 'forward') {
        if (stepNum === QUIZ_PHASE1_END) {
          router.push('/quiz/wellness')
        } else if (stepNum >= TOTAL_STEPS) {
          router.push('/quiz/result')
        } else {
          const next = stepNum + 1
          setStepNum(next)
          window.history.pushState(null, '', `/quiz/${next}`)
          window.scrollTo(0, 0)
        }
      } else {
        if (stepNum <= 1) {
          ...
          setStepNum(prev)
          window.history.pushState(null, '', `/quiz/${prev}`)
          window.scrollTo(0, 0)
        }
      }
    },
    [stepNum, router, setDirection]
  )
